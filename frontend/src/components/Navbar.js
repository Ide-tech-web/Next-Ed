...
const Navbar = () => {
  ...
  // Level switcher — temporary view state, not persisted to DB
  const handleSwitchLevel = (level) => {
    localStorage.setItem('viewLevel', level);
    setShowLevelSwitcher(false);
    closeMenu();
    // Trigger re-render on StudentDashboard
    if (location.pathname === '/study-hub') {
      window.dispatchEvent(new Event('viewLevelChanged'));
    } else {
      navigate('/study-hub');
    }
  };
  ...
  const handleResetLevel = () => {
    localStorage.removeItem('viewLevel');
    setShowLevelSwitcher(false);
    closeMenu();
    if (location.pathname === '/study-hub') {
      window.dispatchEvent(new Event('viewLevelChanged'));
  ...
  const currentViewLevel = localStorage.getItem('viewLevel');
            ...
            <div className="menu-content" onClick={(e) => e.stopPropagation()}>
              {user && (
                <>
                  ...
                  <div className="menu-user-info">
                    ...
                    <div>
                      ...
                      <span className="menu-user-role">
                        ...
                        {currentViewLevel && user.role !== 'ADMIN' ? ` (${t('viewing') || 'Viewing'}: L${currentViewLevel})` : ''}
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Level Switcher — for Students and Delegates */}
                  {(user.role === 'STUDENT' || user.role === 'DELEGATE') && (
                    <>
                      <div className="menu-divider"></div>
                      <button
                        className="menu-link"
                      ...
                      >
                        🔀 {t('switchLevel') || 'Switch Level'} {showLevelSwitcher ? '▲' : '▼'}
                      </button>
                      {showLevelSwitcher && (
                        <div style={{ paddingLeft: '1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                          {[1, 2, 3].map(level => (
                            <button
                              ...
                              style={{
                                background: currentViewLevel === String(level) ? 'rgba(42,157,143,0.2)' : 'none',
                                border: currentViewLevel === String(level) ? '1px solid rgba(42,157,143,0.4)' : '1px solid transparent',
                                color: currentViewLevel === String(level) ? 'var(--primary)' : 'var(--text-secondary)',
                            ...
                            >
                              📘 {t('level') || 'Level'} {level} {String(user.level) === String(level) ? `(${t('myLevel') || 'My Level'})` : ''}
                            </button>
                          ))}
                          {currentViewLevel && (
                            <button
                            ...
                            >
                              ↩️ {t('resetToMyLevel') || 'Reset to My Level'}
                            </button>
                          )}
                        </div>
                      )}
